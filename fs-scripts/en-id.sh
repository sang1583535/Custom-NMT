...
if [ $? -eq 1 ]; then
    ...
    rm -rf $fs_data
    python3.8 $FSROOT/preprocess.py --source-lang $src --target-lang $tgt \
...
if [ $? -eq 1 ]; then
    ...
    do
        ...
        python3.10 $FSROOT/interactive.py $fs_data \
            ...
            --user-dir $LIB/rdrop \
            --remove-bpe \
            --tokenizer moses --bpe subword_nmt --bpe-codes $BPE_CODE > $fs_res/generate-test-$c.txt
        grep ^D- $fs_res/generate-test-$c.txt | sort -V | cut -f3 | perl $DETOKENIZER -threads 8 -l $tgt > $fs_res/translations-$c.txt
