...
if [ $? -eq 1 ]; then
    ...
    do
        ...
        echo "checkpoint-$c inference completed"
        sleep 10
    done
fi
...
if [ $? -eq 1 ]; then
    ...
    cat "$tst/$tgt2.devtest" | python3.10 $TOKENIZER_CUSTOM > "$fs_res/reference.tok"
    ...
    for c in "${checkpoints[@]}";
    do
        cat "$fs_res/translations-$c.txt" | python3.10 $TOKENIZER_CUSTOM > "$fs_res/translations-$c.tok"

        bleu=$(cat "$fs_res/translations-$c.tok" | sacrebleu "$fs_res/reference.tok" -b -l "$lang" -tok none -m bleu)
        chrf=$(cat "$fs_res/translations-$c.tok" | sacrebleu "$fs_res/reference.tok" -b -l "$lang" -tok none -m chrf --chrf-word-order 2)
